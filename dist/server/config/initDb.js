...
export const initializeDatabase = async () => {
    try {
        ...
        const tenantResult = await DatabaseManager.query(`INSERT INTO tenants (name, api_key) 
       ...
       RETURNING id`);
        if (tenantResult.rows.length > 0) {
            ...
            await DatabaseManager.query(`INSERT INTO users (tenant_id, email, password_hash, role) 
         VALUES ($1, 'admin@example.com', 'default-hash', 'admin') 
         ON CONFLICT (email, tenant_id) DO NOTHING`, [tenantId]);
